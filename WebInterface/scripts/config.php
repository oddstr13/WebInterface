...
$maxSellPrice = 1000; //this is per item
$marketDaysMin	= 120; //number of past days to take the average of the sales for to work out market price (bigger number for smaller servers)
...
$auctionLength = 120; //days before auction is ended, and items returned to owner
$auctionFee = 0.0001; //% of the market price you are charged to auction the item
$chargeAdmins = true; //whether web admins get charged fees

/* Storage config */

$costPerItemPerDay = 0; //cost to store 1 item for a day, per item not per stack eg. 64 x Wool + 10 x Stone = 14.8 per day when cost = 0.20
$numberOfChecksPerDay = 1; //checks take a while, for whoever happens to open the page when a check is needed, but more checks make it harder to avoid the cost.
